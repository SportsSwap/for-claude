...
module.exports = function (pres, ctx) {
  ...
  // ---------------------------------------------------------------- 7. FRAMEWORK
  {
    ...
    s.addNotes("S7 | 12s | SPEAKER C | do NOT teach Porter and Kramer to the Shared Value Project\n\n\"Porter and Kramer describe three levels. Almost every mining ESG story lives at level two. We're claiming level one, and here's what that's worth.\"\n\nThis is already the floor. Nothing to cut.");
  ...
  {
    const s = pres.addSlide(); bg(s); nav(s, "Shared Value");
    s.addNotes("S8 | 50s | SPEAKER C | walk the four boxes left to right as ONE chain; each arrow is a \"which means\"\n\n\"Calcination is the most energy-intensive step in the chain. PLS owns a Calix electric calciner outright, opened in June, that cuts those emissions by more than 80%. Which means the tonne has a low, DECLARABLE carbon footprint.\"\n\n\"That word is doing the work. From 2027 the EU Battery Regulation requires a carbon footprint declaration and a battery passport. US foreign-entity rules point the same way. A tonne that can't be declared can't enter those chains at any price.\"\n\n\"So this is a market-access story before it is a cost story. PLS is building tonnes that qualify where competitors' tonnes won't.\"\n\nLAND: declarable. Say the word three times.  CUT IF BEHIND: the US foreign-entity clause.");
  ...
  {
    const s = pres.addSlide(); bg(s); nav(s, "Shared Value");
    s.addNotes("S9 | 50s | SPEAKER C\n\n\"Two levers, and we'll label them honestly.\"\n\n\"First, ore sorting. The A$103m P680 project installed a TOMRA sorter, over a thousand tonnes an hour. It rejects barren rock before crushing, so fewer tonnes get milled for the same lithium. FY26 unit costs fell fifty-eight dollars a tonne. We attribute about thirty of that to sorting.\"\n\n\"Second, power. Pilgangoora runs its own generation. Gas has displaced diesel, solar and storage are next. About eighteen dollars a tonne.\"\n\n\"Forty-eight dollars a tonne between them. Both are things a good operator would do without an ESG label, so we don't claim them as a differentiator. We claim them as cash.\"\n\nLAND: A$48/t, and that we are not overclaiming it.  CUT: the power detail, say \"power adds eighteen\".");
  ...
  {
    const s = pres.addSlide(); bg(s); nav(s, "Shared Value");
    s.addNotes("S10 | 30s | SPEAKER C | do not apologise. This is a scoring slide\n\n\"We went looking for holes in PLS's shared-value story and we found six. The net-zero target is a DECADE, not a year. The decade commencing 2040. There's no SBTi validation and no Scope 3 disclosure, on a company whose product is consumed downstream. The tailings method isn't disclosed either, so we carry it as an unpriced risk rather than assume it's fine.\"\n\n\"We took the haircut before we built the number.\"\n\nLAND: a decade is not a date.  CUT IF BEHIND: the tailings sentence.\nHANDOVER: \"So here's the number after that haircut.\"");
  ...
  {
    const s = pres.addSlide(); bg(s); nav(s, "Shared Value");
    s.addNotes("S11 | 62s | SPEAKER D | the most important slide in the deck. Answer double-counting before it is asked\n\n\"First, what this bridge is not. Nothing on it is added to the DCF. Ore sorting and the power strategy are already inside the five sixty-nine cost base the model discounts. This is an attribution, not an addition. We're asking where the value comes from, not adding it twice.\"\n\n\"Forty-three cents a share. Thirty cents is the two cost levers. Thirteen cents is a P2000 permitting delay we think social licence avoids.\"\n\n\"Now the arithmetic that reframes the pitch. Measured against the six-fourteen target, forty-three cents is 4.6%. A rounding adjustment. Measured against the sixty-six cents of upside, it's 66%. Most of the investment case.\"\n\n\"And there's a further thirty-two cents of downstream value we've deliberately left OUTSIDE the target. Nothing here depends on it landing.\"\n\nLAND: attribution not addition, then 4.6% versus 66%.  CUT: the thirty-two cents, keep it for Q&A.");
